<x-app-layout>
    <div class="d-flex align-items-start">
                                            ...
                                            @foreach($certificates as $index => $certificate)
                                                <tr>
                                                    ...
                                                    <td class="text-center">
                                                        <a href="{{ route('certificate.show', $certificate->id) }}" class="me-1" style="cursor: pointer">
                                                            <i class="fas fa-circle-info" style="color: #FF7506"></i>
                                                        </a>
                                                        <form action="{{ route('certificate.destroy', $certificate->id) }}" method="POST" style="display:inline;" id="deleteCertificateForm-{{ $certificate->id }}">
                                                            @csrf
                                                            @method('DELETE')
        ...
        <div class="d-flex flex-column justify-content-between ms-4 bg-white sm:rounded-lg" id="addNewButtonContainer">
            <a href="{{ route('certificate.create') }}" class="btn btn-white d-flex flex-column align-items-center justify-content-center border-0 p-3" style="color: #FF7506; border-color: #FF7506; width: 80px; text-align: center;" id="addCertificateButton">
